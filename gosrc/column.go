...
type Column interface {
	...
	Value(h api.SQLHSTMT, idx int) (Value, error)
}
...
func (c *BaseColumn) Value(buf []byte) (Value, error) {
	// fmt.Println("in basecolumns value")
...
func NewBindableColumn(b *BaseColumn, ctype api.SQLSMALLINT, bufSize int) *BindableColumn {
	// fmt.Println("in new bindable columns")
	b.CType = ctype
...
func NewVariableWidthColumn(b *BaseColumn, ctype api.SQLSMALLINT, colWidth api.SQLULEN) (Column, error) {
	// fmt.Println("in new variable width columns")
...
func (c *BindableColumn) Bind(h api.SQLHSTMT, idx int) (bool, error) {
	// fmt.Println("in bind of bindable column")
...
func (c *BindableColumn) Value(h api.SQLHSTMT, idx int) (Value, error) {
	// fmt.Println("in bindable columns value")
	if !c.IsBound {
...
func (c *NonBindableColumn) Value(h api.SQLHSTMT, idx int) (Value, error) {
	var l BufferLen
	var total []byte
	b := make([]byte, 1024)
loop:
	for {
		...
		case api.SQL_SUCCESS_WITH_INFO:
			...
			if l != api.SQL_NO_TOTAL {
				// odbc gives us a hint about regosrcing data,
