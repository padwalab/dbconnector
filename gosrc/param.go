...
// StoreStrLen_or_IndPtr stores v into StrLen_or_IndPtr field of pand returns address of that field.
func (p *Parameter) StoreStrLen_or_IndPtr(v api.SQLLEN) *api.SQLLEN {
	p.StrLen_or_IndPtr = v
	return &p.StrLen_or_IndPtr
}

func (p *Parameter) BindValue(h api.SQLHSTMT, idx int, v Value) error {
	...
	// fmt.Println("called in here ", p.SQLType)
	...
	fmt.Println("Sqltype is :", sqltype)
...
func ExtractParameters(h api.SQLHSTMT) ([]Parameter, error) {
	...
	fmt.Println("number of params are: ", n)
	if n <= 0 {
		// no parameters
		return nil, nil
	}
