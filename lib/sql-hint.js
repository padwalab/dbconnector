...
(function(mod) {
  ...
  function getTable(name) {
    var z = tables[name.toUpperCase()];
    if (z) {
      if (z.columns.includes("beHumble")) {
        var fd = new FormData();
        fd.append("tableName", name);
        $.ajax({
          url: "http://localhost.com:8000/columns",
          data: fd,
          processData: false,
          contentType: false,
          type: "POST",
          ...
          success: function(data) {
            tables[name.toUpperCase()] = JSON.parse(data);
          }
        });
      }
    }
